...
class FitnessTestAnalyticsController extends Controller
{
    ...
    public function stats(Request $request): JsonResponse { return response()->json(['data' => $this->analytics->stats($this->analytics->filters($request->query()))]); }
    public function trends(Request $request): JsonResponse { return response()->json(['data' => $this->analytics->trends($this->analytics->filters($request->query()))]); }
    public function checkpoints(Request $request): JsonResponse { return response()->json(['data' => $this->analytics->checkpoints($this->analytics->filters($request->query()))]); }
    public function coverage(Request $request): JsonResponse { return response()->json(['data' => $this->analytics->coverage($this->analytics->filters($request->query()))]); }
